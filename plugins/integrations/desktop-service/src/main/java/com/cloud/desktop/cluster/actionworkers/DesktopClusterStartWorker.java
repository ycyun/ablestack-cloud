...
public class DesktopClusterStartWorker extends DesktopClusterResourceModifierActionWorker {
    ...
    private boolean setupDesktopClusterNetworkRules(Network network, UserVm worksVm, IpAddress publicIp) throws ManagementServerException {
        ...
        if (egress) {
            try {
                firewall = provisionFirewallRules(publicIp, owner, CLUSTER_USER_PORTAL_PORT, CLUSTER_ADMIN_PORTAL_PORT);
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info(String.format("Provisioned firewall rule to open up port %d to %d on %s for Desktop cluster : %s", CLUSTER_USER_PORTAL_PORT, CLUSTER_ADMIN_PORTAL_PORT, publicIp.getAddress().addr(), desktopCluster.getName()));
                }
            } catch (NoSuchFieldException | IllegalAccessException | ResourceUnavailableException | NetworkRuleConflictException e) {
            ...
            if (firewall) {
                ...
                try {
                    portForwarding = provisionPortForwardingRules(publicIp, network, owner, worksVm, CLUSTER_ADMIN_PORTAL_PORT, CLUSTER_USER_PORTAL_PORT);
